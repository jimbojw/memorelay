...
const ALTERNATIVE_SIGNED_EVENT = Object.freeze({
...
describe('EventsDatabase', () => {
    describe('addEvent', () => {
        it('should return true for a previously unknown event', () => {
            const coordinator = new events_database_1.EventsDatabase();
            expect(coordinator.addEvent(EXAMPLE_SIGNED_EVENT)).toBe(true);
        });
        it('should be able to add events with same created_at stamp', () => {
            const coordinator = new events_database_1.EventsDatabase();
            expect(coordinator.addEvent(EXAMPLE_SIGNED_EVENT)).toBe(true);
            const collidingEvent = (0, signed_test_event_1.createSignedTestEvent)({
        ...
        it('should throw when adding an invalid event object', () => {
            ...
            }).toThrow(bad_event_error_1.BadEventError);
            expect(() => {
                coordinator.addEvent(undefined);
            }).toThrow(bad_event_error_1.BadEventError);
            expect(() => {
                coordinator.addEvent(EXAMPLE_SIGNED_EVENT.id);
            }).toThrow(bad_event_error_1.BadEventError);
        });
    });
    describe('hasEvent', () => {
    ...
    describe('matchFilters', () => {
        it('should return an empty array when no events are known', () => {
            ...
            expect(coordinator.matchFilters([{ ids: ['6'] }, { kinds: [1] }])).toEqual([]);
        });
        ...
        it('should find only events up to the limit', () => {
            ...
            expect(coordinator.matchFilters([{ limit: 1 }])).toEqual(EXPECTED_RESULTS);
        });
